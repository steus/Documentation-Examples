...
static NSString * const kAdMobJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_admobInjection
{
	...
	[html appendString:kAdMobJavascript];
...
{
	// attempt to make well-formed HTML and inject in our AdMob bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _admobInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to AdMob support.",url);
				}
			}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
	...
	[[self proxy] replaceValue:url forKey:@"url" notification:NO];
